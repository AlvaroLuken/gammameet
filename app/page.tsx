  ...
  return (
    <main className="min-h-screen bg-zinc-50 dark:bg-black text-zinc-900 dark:text-white transition-colors">
      ...
      <section className="max-w-4xl mx-auto px-8 py-28 text-center space-y-8">
        ...
        <SignInButton label="Get started free" large />
      ...
      <section className="py-24 px-8 text-center space-y-6">
        ...
        <SignInButton label="Start for free" large />
      </section>
...
function SignInButton({ label, large }: { label: string; large?: boolean }) {
  return (
    <form
      action={async () => {
        "use server";
    ...
    >
      <button
        type="submit"
        className={`inline-flex items-center gap-3 bg-white text-black font-semibold rounded-full transition-colors cursor-pointer hover:bg-zinc-100 ${
...
function GoogleIcon() {
  return (
    <svg className="w-5 h-5" viewBox="0 0 24 24">
      ...
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.47 2.18 7.07l3.66 2.84z" />
    </svg>
  );
}
